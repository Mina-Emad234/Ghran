 ...
 *
 *
 * FREE BEER LICENSE VERSION 2.02
 *
 ...
 *
 * 0.2.2    Bug fixes
 *          - Backward compatibility with jQuery 2.2.x
 *          - Added test file to the archive
 *          - Bug fix: The right arrow appeared when it wasn't necessary (thanks to <admins at unix dot am>)
 *
 * 0.2.2    Bug fixes
 *          - Backward compatibility with jQuery 2.2.x (thanks to Andy Mull for compatibility with jQuery >= 2.2.4)
 ...
 *
 *          note: The css file has not changed therefore it is still versioned 0.2.0
 *
 * 0.2.0    First release of the plugin. Supports:
...
$.fn.hoverscroll = function(params) {
	...
	this.each(function() {
		...
		if (!params.vertical) {
			...
			$this.children().each(function() {
				...
				if ($(this).outerWidth) {
					...
					// jQuery < 2.2.x backward compatibility patch
		...
		function setArrowOpacity() {
			...
			// Thanks to <admins at unix dot am> for fixing the bug that displayed the right arrow when it was not needed
			var done = false;
			if (opacity <= 0) {
		 ...
		 *
		 * @param direction {Integer}	Direction of the displacement, either -2|2
		...
		// jQuery 2.2.x backward compatibility, thanks to Andy Mull!
